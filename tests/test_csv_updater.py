...
class TestCSVUpdater:
    def test_update_existing_upn(self, sample_csv):
        ...
        assert updater.update_twist(27870, -0.89) is True
        updater.save()
        df = pd.read_csv(sample_csv)
        assert df.loc[df["UPN"] == 27870, "Twist_Deg"].values[0] == -0.89
    ...
    def test_multiple_updates_single_save(self, sample_csv):
        updater = CSVUpdater(str(sample_csv))
        updater.update_twist(27870, -0.89)
        updater.update_twist(7770, 2.65)
        updater.save()
        df = pd.read_csv(sample_csv)
        assert df.loc[df["UPN"] == 27870, "Twist_Deg"].values[0] == -0.89
        assert df.loc[df["UPN"] == 7770, "Twist_Deg"].values[0] == 2.65

    def test_overwrites_existing_twist_value(self, sample_csv):
        updater = CSVUpdater(str(sample_csv))
        updater.update_twist(27870, 1.0)
        updater.update_twist(27870, -0.89)  # overwrite
        updater.save()
        df = pd.read_csv(sample_csv)
        assert df.loc[df["UPN"] == 27870, "Twist_Deg"].values[0] == -0.89

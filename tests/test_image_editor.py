...
class TestAnnotateImage:
    def test_returns_image(self, sample_image):
        result = annotate_image(str(sample_image), -0.89)
    ...
    def test_original_file_not_modified(self, sample_image):
        ...
        annotate_image(str(sample_image), -0.89)
    ...
    def test_handles_negative_new_twist(self, sample_image):
        result = annotate_image(str(sample_image), -1.86)
        assert isinstance(result, Image.Image)

    def test_handles_positive_new_twist(self, sample_image):
        result = annotate_image(str(sample_image), 2.15)
        assert isinstance(result, Image.Image)
    ...
    def test_handles_png(self, tmp_path):
        ...
        img.save(str(path))
        result = annotate_image(str(path), -0.89)
        assert isinstance(result, Image.Image)
